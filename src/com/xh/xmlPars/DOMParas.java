...
class DOMParas extends Paras {
	DocumentBuilder documentBuilder;

	public DOMParas() throws Exception {
	...
	public List<NodeTree> paras(InputSource is) {
		...
		if (is == null)
			return null;
		try {
			return document2list(documentBuilder.parse(is));
		} catch (Exception e) {
			// TODO: handle exception
		}
		return null;
	 ...
	 */
	@Override
	public String list2string(List<NodeTree> nodeTree) {
		try {
			if (nodeTree == null)
				return null;
			Transformer transformer = getTransformer();
